...
/** The org an API key resolves to — only the fields this surface reads. */
interface Publisher {
  id: string;
  bonuses: unknown;
}
...
/**
 * What the Partner API means on top of the repository: body validation, risk grading, campaign
 * offers, response shapes, and the decision series that is the only symptom of a misconfigured
 * integration. Methods take the raw `Authorization` header — no guard, the API key is the identity.
 */
@Injectable()
export class PartnerService {
  ...
  // Parse and bound the claim half of a body arriving from another company's server. The old iOS
  // fingerprint fields are ignored rather than rejected, so an un-upgraded SDK degrades to "no claim".
  private readCarried(b: Record<string, unknown>): Carried {
    ...
    // Wrapped back into referrer shape so there is one definition of it. Malformed is a 400: a
    // silent drop turns a broken integration into zero attribution.
    ...
    // Believed because it decides nothing: every carrier pays the same fee. Absent or unknown means
  ...
  async firstOpen(auth: string, b: FirstOpenBody) {
    ...
    // Asserted by the SDK, never observed, so graded not trusted: `emulator` blocks; `rooted`/`vpn`
    // are recorded only, their honest populations being too large to refuse. Stored on accepted
    // installs too — the pattern worth finding is the one that got paid.
    const risk = {
    ...
    // One record per first-open, refused or bound: a misconfigured store target errors nowhere and
    // writes no rows, so this series is the only symptom.
    ...
    // What *this campaign* advertises, not everything the publisher runs — the artwork was printed off these.
    const granted = campaignBonuses(publisher.bonuses, 'acquisition', scan.bonus_types);

    return {
      attributed: true,
      /** present at signup. Not a credential and not spendable — it names an install. */
      install_id,
      campaign_id: scan.campaign_id,
      campaign_name: scan.campaign_name,
      /** carrier that brought the claim id back: referrer | appclip | pasteboard */
      match_method,
  ...
  async claim(auth: string, b: ClaimBody) {
    ...
    // Bounded before parsing or storing — attacker-shaped strings from another server.
    ...
    // Wrapped back into referrer shape so there is one definition of an issued code. Malformed is a
    // 400, never a fallthrough to the acquisition path.
    ...
    // Every refusal funnels through here, including ones from a rolled-back transaction.
    ...
    // The fee buys an *acquisition*, and only the publisher knows an account is new. Refused before
    // the install is spent: the UNIQUE on (campaign, ref) catches a repeated ref, this a fresh one.
    ...
    if (result.status === 'attributed') {
      ...
      return {
        ...
        fee,
        /** what landed in the publisher's account: fee minus the platform's cut */
        publisher_net: net,
        platform_fee: cut,
        ...
        bonuses: granted,
        bonus_label: bonusLabel(granted),
        /** false on the first answer for this user; see the replay note below */
        replay: false,
      };
    }

    // Replay: already attributed, i.e. the first call succeeded and its response was lost. A 409
    // would look like something to reconcile by hand, so the original answer is replayed verbatim.
    const prior = await this.repo.priorAcquisition(result.campaign_id, publisher_user_ref);
    // Gone only if the campaign was deleted between the calls; nothing left to replay.
    ...
    // Counted separately: a replay moved no money, and a climbing rate means retries firing.
    recordDecision(
      'claim',
  ...
  // Engagement payout: a repeat purchase, paid on the code that proves it. Not routed through the
  // acquisition machinery — that is all device recognition, which has nothing to answer here.
  private async claimPurchase(publisher: Publisher, code: string, publisher_user_ref: string) {
    ...
    if (result.status === 'attributed') {
      ...
      return {
        ...
        publisher_net: net,
        platform_fee: cut,
        /** always 0, with a null deadline: an engagement payout settles in one step */
        pending_fee: 0,
    ...
    // Same user: the first call's response was lost, so replay. Different user: somebody else's
    // boarding pass, forwarded or shared — replaying would hand them a reward the first earned.
